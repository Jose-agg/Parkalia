...
public class VentanaCambiosEntrada extends JDialog {
	...
	public VentanaCambiosEntrada(VentanaPrincipal vp, String datos, String tipoAccion) {
	...
	private JPanel getPnDatos() {
		if (pnDatos == null) {
			...
			pnDatos.add(getPnDias(), BorderLayout.NORTH);
	...
	private JPanel getPnDias() {
		if (pnDias == null) {
			...
			pnDias.add(getPnSubDias(), BorderLayout.WEST);
			pnDias.add(getSpinDias(), BorderLayout.EAST);
		}
		return pnDias;
	}

	private JPanel getPnSubDias() {
		if (pnSubDias == null) {
			pnSubDias = new JPanel();
			pnSubDias.add(getLblDias());
		}
		return pnSubDias;
	}
	...
	private JSpinner getSpinDias() {
		if (spinDias == null) {
			...
			spinDias.setModel(new SpinnerNumberModel(new Integer(5), new Integer(1), null, new Integer(1)));
		}
		return spinDias;
	}
	...
	private void mostrarDatos(String datos) {
		...
		txtDenominacion.setText(original[0]);
		txtFechaAntigua.setText(original[1]);
		spinDias.setModel(new SpinnerNumberModel(new Integer(Integer.valueOf(original[2])), new Integer(1), null,
				new Integer(1)));
		spinCantidadesAdultos.setModel(new SpinnerNumberModel(new Integer(Integer.valueOf(original[3])), new Integer(0),
				null, new Integer(1)));
		spinCantidadesKids.setModel(new SpinnerNumberModel(new Integer(Integer.valueOf(original[4])), new Integer(0),
				null, new Integer(1)));
		btnVariable.setText(tipoAccion);
		if (tipoAccion.equals("Borrar")) {
			btnVariable.setMnemonic('B');
			spinDias.setEnabled(false);
			...
			lblDias.setDisplayedMnemonic('N');
	...
	private void borrarEntrada() {
		...
		if (JOptionPane.YES_OPTION == respuesta) {
			vp.getManager().eliminarReservaEntrada(original);
			dispose();
		}
	}
	...
	private void modificarEntrada() {
		...
		if (JOptionPane.YES_OPTION == respuesta) {
			...
			vp.getManager().modificarReservaEntrada(original, fecha, numDias, numAdultos, numKids);
			dispose();
		}
	}
